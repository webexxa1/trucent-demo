        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6">
            ...
            <div>
                      ...
                      <div className="lg:col-span-1">
                        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 p-4 bg-white/70 dark:bg-zinc-900/70">
                          ...
                          <ul className="space-y-2 text-sm">
                            <li className="flex justify-between"><span>Baseline Monthly</span><span>${baselineMonthly.toLocaleString()}</span></li>
                            <li className="flex justify-between"><span>After Monthly</span><span>${afterMonthly.toLocaleString()}</span></li>
                            <li className="flex justify-between"><span>Monthly Savings</span><span style={{color: BRAND.green}}>${roi.monthlySavings.toLocaleString()}</span></li>
                            <li className="flex justify-between"><span>Annual Savings</span><span>${roi.annualSavings.toLocaleString()}</span></li>
                            <li className="flex justify-between"><span>Payback</span><span>{Number.isFinite(roi.paybackMonths) ? `${roi.paybackMonths.toFixed(1)} months` : '—'}</span></li>
                            <li className="flex justify-between"><span>3-Year NPV</span><span className={roi.npv3>=0?"":"text-red-600"}>${roi.npv3.toLocaleString(undefined,{maximumFractionDigits:0})}</span></li>
                          </ul>
                          <button
              ...
              {/* Inline AI Chatbot */}
              <div className="mt-8">
                <div className="rounded-2xl border border-zinc-200/60 dark:border-zinc-800 p-4 bg-white/70 dark:bg-zinc-900/70 shadow-sm">
                  <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                    <Sparkles className="w-4 h-4" style={{ color: BRAND.blue }} />
                    AI Chatbot
                  </h3>
                  <div className="h-64 overflow-y-auto mb-3 border rounded-lg p-2 bg-white dark:bg-zinc-800">
                    {messages.map((m, i) => (
